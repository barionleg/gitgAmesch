...
MeshQt::MeshQt( const QString&           rFileName,           //!< File to read
	...
	mDialogNprSetting = new QGMDialogNprSettings( setmMainWindow );
	...
	//! \bug Emitting inside constructor has no effect
	emit sFileChanged( QString( getFullName().c_str() ), QString( getBaseName().c_str() ) );

	QSettings settings;
	settings.setValue( "lastPath", QString( getFileLocation().c_str() ) );
	...
	QObject::connect( mDialogNprSetting, &QGMDialogNprSettings::valuesChanged, this, &MeshQt::NPRSettingsChanged  );
	QObject::connect( &mDialogTransparencySettings,
	                  SIGNAL(valuesChanged()),
	                  this,
	                  SLOT(TransparencySettingsChanged())
	                );
...
bool MeshQt::writeFileUserInteract() {
	...
	QString fileSuggest = QString( getBaseName().c_str() );
...
void MeshQt::showNPRSettings() {
	mDialogNprSetting->init( this, mRenderColors );
	mDialogNprSetting->show();
}

void MeshQt::NPRSettingsChanged()
{
    emit updateGL();
}
...
{
    mDialogTransparencySettings.init(this , mOpenGLContext->format().majorVersion() > 4 ||
                                     (mOpenGLContext->format().majorVersion() == 4 && mOpenGLContext->format().minorVersion() >= 3));
    mDialogTransparencySettings.exec();
}

void MeshQt::TransparencySettingsChanged()
{
    emit updateGL();
}
