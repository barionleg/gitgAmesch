...
bool QGMDialogEnterText::checkAndConvertTextVecDouble(
	...
	bool            someTokensAreNoDouble = false;
        QStringList     someTokens  = rEnteredText.simplified().split( " ", QString::SplitBehavior::SkipEmptyParts );
		...
		if( !convertOk ) {
			...
			if( countColons == 0 ) {
				...
				continue;
                        ...
                        QStringList colonTokens = someTokens.at( i ).split( ":", QString::SplitBehavior::SkipEmptyParts );
...
void QGMDialogEnterText::accept() {
	...
	//! *) Arrays of
        QStringList someTokens  = someText->text().split( " ", QString::SplitBehavior::SkipEmptyParts );
	int         someTokenNr = someTokens.size();
