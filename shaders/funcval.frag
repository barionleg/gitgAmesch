...
float getIsoLineAlpha( float vertexFuncVal, grFuncValIsoLineParams rFuncValIsoLineParams ) {
	...
	float funcValModIsoDist = vertexFuncVal + isoOffset - isoDist*floor( ( vertexFuncVal + isoOffset ) / isoDist ); // Not working: modf( abs( vertexFuncVal ), isoDist );
	float funcValDxy = length( vec2( dFdx( vertexFuncVal ), dFdy( vertexFuncVal ) ) );
	float funcValRemainder = funcValModIsoDist - pixelWidth*funcValDxy;
	float alphaValue = 0.0;
	if( funcValRemainder <= 0.0 ) {
		alphaValue = -funcValRemainder/(pixelWidth*funcValDxy);
	}
	float funcValRemainderPos = funcValModIsoDist + pixelWidth*funcValDxy - isoDist;
	if( funcValRemainderPos >= 0.0 ) {
		alphaValue = +funcValRemainderPos/(pixelWidth*funcValDxy);
	}
	return alphaValue;
}
...
void main(void) {
	...
	// +++ Invert color, when request e.g. for normals:
		...
		vec4 fixedWorldLight = getLightAmount( gVertex.FixedWorld_L, -normal, gVertex.FixedWorld_halfVector, FixedWorld_DiffuseProduct, FixedWorld_SpecularProduct );
		// ++++ Sum up all parts of the light:
		vec4 lighting = AmbientProduct + fixedCamLight + fixedWorldLight;


		vec4 outputColor  = mix(colorSolidBack, colorSolidBack * lighting, bvec4(backLighting));
	// ++++ Isolines based on the function value:
		if( funcValIsoLineParams.mIsoLinesShow && funcValIsoLineParams.mIsoLinesOnly ) {
			outputColor.a = getIsoLineAlpha( gVertex.vertexFuncVal, funcValIsoLineParams );
