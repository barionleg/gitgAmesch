...
void main(void) {
	...
	// ++++ Label ID mapped to color:
		...
		if( uRenderColor == 3 ) {
			// Shade labeled areas:
			float labelNrShifted = round(gVertex.labelNr) + uLabelCountOffset;
			...
			if( fwidth( gVertex.labelNr ) > 0.0001 ) {	//cannot compare to 0.0 because of noise generated by fragment interpolation
