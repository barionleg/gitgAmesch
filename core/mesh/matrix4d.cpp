...
//!calculates the eigen values with QR decomposition
//! source: https://github.com/QuantitativeBytes/qbLinAlg
bool Matrix4D::calculateEigenValues(std::vector<double> *eigenValues)
{
    Matrix4D identityMatrix = Matrix4D(_MATRIX4D_INIT_IDENTITY_,1.0);
    Matrix4D Q();
    Matrix4D R();
    Matrix4D inputMatrix = *this;
    //!\todo define max iterations const and find a good value
...
void Matrix4D::calcQRDecomposition(Matrix4D *inputMatrix, Matrix4D *Q, Matrix4D *R)
{
    // Create a vector to store the P matrices for each column.
    std::vector<Matrix4D> Plist;
    //Loop through each column
    for (unsigned int i = 0; i < 4; i++){
        // Create the a1 and b1 vectors.
        // a1 is the column vector from A.
        // b1 is the vector onto which we wish to reflect a1
        std::vector<double> a1(4-i);
        std::vector<double> b1(4-i);
        for(unsigned int j=i; j<4;j++){
            a1[j-i] = inputMatrix->get(j,i);
            b1[j-i] = static_cast<double>(0.0);
        }
        b1[0] = static_cast<double>(1.0);

        //compute the norm of a1
        double a1Norm = 0.0;
        for(double element: a1){
            a1Norm += element*element;
        }
        a1Norm = sqrt(a1Norm);
        ...
        std::vector<double> u(4-i);
        for(unsigned int k=0; k<(4-i); k++){
            u[k] = a1[k] - (sgn * a1Norm * b1[k]);
        }

        //compute the n-vector  (u normalized)
        std::vector<double> n(4-i);
        double uNorm = 0.0;
        for(double element: u){
            uNorm += element*element;
        }
        uNorm = sqrt(uNorm);
