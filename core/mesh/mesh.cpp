...
//Wedge extraction uses these includes
...
double Mesh::weExComputeSquaredDistanceBetweenTwoVertices(Vertex* &vertexNo1, Vertex* &vertexNo2){
...
double Mesh::weExComputeSquaredDistanceBetweenTwoNormalsTreatedAsVertices(Vertex* &vertexNo1, Vertex* &vertexNo2){
...
double Mesh::weExComputeSquaredDistanceBetweenTwoNormalsTreatedAsVertices(Vertex* &vertexNo1, vector<double> &normalComponents){
...
void Mesh::weExRandomlyChooseVerticesFromVector(vector<Vertex*> &inputVector, vector<Vertex*> &outputVector, int howManyVerticesWanted){
...
void Mesh::weExAssignNormalsToNearestMean(vector<int> &currentClustering, vector<Vertex*> &verticesWithCurrentLabel, vector<vector<double>> &normalMeanComponents){

	for(int i=0; i<verticesWithCurrentLabel.size(); i++){

		double distanceToNormal1 = weExComputeSquaredDistanceBetweenTwoNormalsTreatedAsVertices(verticesWithCurrentLabel[i], normalMeanComponents[0]);
		double distanceToNormal2 = weExComputeSquaredDistanceBetweenTwoNormalsTreatedAsVertices(verticesWithCurrentLabel[i], normalMeanComponents[1]);
		double distanceToNormal3 = weExComputeSquaredDistanceBetweenTwoNormalsTreatedAsVertices(verticesWithCurrentLabel[i], normalMeanComponents[2]);
...
void Mesh::weExAssignNormalsToNearestMean(vector<int> &currentClustering, vector<Vertex*> &verticesWithCurrentLabel, vector<Vertex*> &threeRandomlyChosenVertices){

	for(int i=0; i<verticesWithCurrentLabel.size(); i++){

		double distanceToNormal1 = weExComputeSquaredDistanceBetweenTwoNormalsTreatedAsVertices(verticesWithCurrentLabel[i], threeRandomlyChosenVertices[0]);
		double distanceToNormal2 = weExComputeSquaredDistanceBetweenTwoNormalsTreatedAsVertices(verticesWithCurrentLabel[i], threeRandomlyChosenVertices[1]);
		double distanceToNormal3 = weExComputeSquaredDistanceBetweenTwoNormalsTreatedAsVertices(verticesWithCurrentLabel[i], threeRandomlyChosenVertices[2]);
...
void Mesh::weExComputeMeans(vector<int> &currentClustering, vector<Vertex*> &verticesWithCurrentLabel, vector<vector<double>> &normalMeanComponents){
...
//! currently uses the square root
double Mesh::weExComputeShortestDistanceBetweenPointAndRay(Vertex* &arbPoint, Vertex* &point1RayStart, Vertex* &point2OnRay){
...
void Mesh::weExGetBorderGroupFromVertexByFeatureVector(Vertex* &finalLineVertex1, Vertex* &finalLineVertex2, Vertex* &finalLineVertex3,
...
void Mesh::weExComputeSquaredDistanceFromTetraederTopToProjectedPointOnLine(Vertex* &arbPoint, Vertex* &point1OnLineTetraederTop, Vertex* &point2OnLine, double &computedSquaredDistance){
...
//One height of the tetraeder is particularly interesting.
//This height is the distance from the tetraeder top, vertex number 1 (Index 0) to the plane
double Mesh::weExComputeTetraederHeight(std::vector<Vertex*> &foundTetraeder){
...
bool Mesh::weExCheckTetraederHeight(vector<Vertex*> &foundTetraeder, double &minimumHeight){
...
void Mesh::weExCheckTriangleFaceOrientation(vector<vector<Vertex*>> &extractedTetraeders, vector<bool> &faceVerdict){
...
void Mesh::weExNormalizeValues(vector<double> &vectorOfDoubles, vector<double> &vectorOfNormalizedDoubles){
...
void Mesh::weExWriteExtractedTetraedersIntoFile(vector<vector<Vertex*>> &extractedTetraeders, string outputFileName){
	...
	if(OutFile.fail()){
		...
		for(int t=0; t < extractedTetraeders.size(); t++){
			...
			//In GigaMesh interface remove the check at Faces - Backface Culling
			//additional faces

			//OutFile << "f " << fComp2 << " " << fComp1 << " " << fComp3 << endl;
			//OutFile << "f " << fComp2 << " " << fComp1 << " " << fComp4 << endl;
			//OutFile << "f " << fComp1 << " " << fComp3 << " " << fComp4 << endl; //may be not counterclockwise
			//OutFile << "f " << fComp4 << " " << fComp1 << " " << fComp3 << endl; //counterclockwise equivalent to line before
			//OutFile << "f " << fComp3 << " " << fComp2 << " " << fComp4 << endl;

			//Adding more faces makes the meshes a little impure, they then basically contain two times the same wedge
...
void Mesh::weExWriteExtractedTetraedersAndMeshIntoFile(	vector<vector<Vertex*>> &extractedTetraeders,
	...
	weExCheckTriangleFaceOrientation(extractedTetraeders, faceVerdict);
	...
	if(OutFile.fail()){
		...
		//int numberOfTetraederVertices = extractedTetraeders.size() * 8;
		...
		//int numberOfTetraederFaces = extractedTetraeders.size() * 8; //Additional faces are added, so no decision about back face and front face needs to be done
		//In GigaMesh interface remove the check at Faces - Backface Culling
		//int numberOfTetraederFaces = numberOfTetraederVertices;
		...
		for(int i=0; i<extractedTetraeders.size(); i++){
			...
			*/
			...
			//Adding both orders for vertices creates the faces both front facing and back facing
...
void Mesh::weExWriteExtractedTetraedersIntoMeshForCollection(	vector<vector<double>> &meshIntrinsicExtractedTetraeders,
...
//TODO: could be slimlined and moved to vertex class
bool Mesh::legacyGetSurroundingVerticesInOrder (list<Vertex*> &adjacentVertsInOrder, Vertex* &pi, bool printDebug) {
...
void Mesh::weExBuildVertexNeighbourLookUpStructure(vector<Vertex*> &mVertices, map<Vertex*,set<Vertex*>> &vertexNeighbourLookUp){

	for(auto pVertexPreC : mVertices){

		list<Vertex*> adjacentVerticesPreC;
		legacyGetSurroundingVerticesInOrder(adjacentVerticesPreC, pVertexPreC, false);
...
bool Mesh::funcWeExSuppNonMax(double NMSDistance){
	...
	weExBuildVertexNeighbourLookUpStructure(mVertices, vertexNeighbourLookUp);
	...
	for(auto pVertex : mVertices){
		...
		if((MSIIcomponent16 > NMSWaterAdditionalInput) && !(succumbed.find(pVertex) != succumbed.end())/*&& (neverWillBeMax < 0.5)*/){
			...
			while(!marchingFrontier.empty() && !biggerValueFound){
				...
				for(auto adjacentVertex : adjacentVertices){
					...
					if(!biggerValueFound && !(checkVisited.find(adjacentVertex) != checkVisited.end())  ){
						...
						if( weExComputeSquaredDistanceBetweenTwoVertices(adjacentVertex, pVertex) <= (squaredDistance)){
...
bool Mesh::funcWeExComputeWatershed(double watershedLimit){

	bool useBasinMerging = true;

	if(NMSWaterAdditionalInput < 0.0){
		useBasinMerging = false;
	}
	...
	for(auto& foundMaximum : foundMaxima){
		...
		list<Vertex*> adjacentVertsInOrder;
		legacyGetSurroundingVerticesInOrder(adjacentVertsInOrder, foundMaximum, false);
	...
	while(!watershedCoast.empty()){
		...
		list<Vertex*> adjacentVertsInOrder;
		legacyGetSurroundingVerticesInOrder(adjacentVertsInOrder, VertexPWithLargestMSIIValue, false);
			...
			if(!mapelement.second.empty()){
				for(auto setelement : mapelement.second){
					if(numberOfVerticesPerLabel[setelement] != 0){
						...
						if(labelQuotient < NMSWaterAdditionalInput){
...
bool Mesh::funcWeExComputeClustering(int numberOfIterations){
	...
	for(int labelIterator=1;labelIterator<=numberOfLabels;labelIterator++){
		...
		if(verticesWithCurrentLabel.size() >= 3){
			...
			weExRandomlyChooseVerticesFromVector(verticesWithCurrentLabel, threeRandomlyChosenVertices, 3);
			...
			weExAssignNormalsToNearestMean(currentClustering, verticesWithCurrentLabel, threeRandomlyChosenVertices);
			...
			for(int iteration=0;iteration<numberOfIterations;iteration++){
				...
				weExComputeMeans(currentClustering, verticesWithCurrentLabel, normalMeans);

				//Determining to which mean a normal is closest is done by a helper method
				weExAssignNormalsToNearestMean(currentClustering, verticesWithCurrentLabel, normalMeans);
...
bool Mesh::funcWeExComputeRANSAC(int numberOfIterations){

	//optimization step for RANSAC, is activated per default
	bool useNMSResultsForTetraederTop = true;
	...
	for(int labelIterator=1;labelIterator<=numberOfLabels;labelIterator++){
		...
		for(auto currentVertex : mVertices){
			...
			if(labelIterator == (int)round(foundLabel)){
				...
				list<Vertex*> adjacentVertsInOrder;
				legacyGetSurroundingVerticesInOrder(adjacentVertsInOrder, currentVertex, false);
		...
		if(verticesBorderGroup.size()>=4){
			...
			if(useNMSResultsForTetraederTop){//so the local maximum should be used as the tetraeder Top
				...
				for(int iteration=0;iteration<numberOfIterations;iteration++){
					...
					weExRandomlyChooseVerticesFromVector(verticesBorderGroup, threeRandomlyChosenBorderVertices, 3);
					...
					for(auto borderVertex : verticesBorderGroup){
						...
						double minimumDistance = min(min(weExComputeShortestDistanceBetweenPointAndRay(borderVertex, chosenTetraederTop, randomlyChosenTetraederVertex1),
													weExComputeShortestDistanceBetweenPointAndRay(borderVertex, chosenTetraederTop, randomlyChosenTetraederVertex2)),
													weExComputeShortestDistanceBetweenPointAndRay(borderVertex, chosenTetraederTop, randomlyChosenTetraederVertex3));
				...
				for(int iteration=0;iteration<numberOfIterations;iteration++){
					...
					weExRandomlyChooseVerticesFromVector(verticesBorderGroup, fourRandomlyChosenBorderVertices, 4);
					...
					for(auto borderVertex : verticesBorderGroup){
						...
						//find minimum distance to one of the rays
						double minimumDistance = min(min(weExComputeShortestDistanceBetweenPointAndRay(borderVertex, randomlyChosenTetraederTop, randomlyChosenTetraederVertex1),
													weExComputeShortestDistanceBetweenPointAndRay(borderVertex, randomlyChosenTetraederTop, randomlyChosenTetraederVertex2)),
													weExComputeShortestDistanceBetweenPointAndRay(borderVertex, randomlyChosenTetraederTop, randomlyChosenTetraederVertex3));
						currentDistanceForThisChoice += minimumDistance;
					}
			...
			for(auto borderVertex : verticesBorderGroup){
				if(		(weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex1) <
						weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex2))				&&
						(weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex1) <
						weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex3))				){
					closestToRay1.push_back(borderVertex);
				} else if	((weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex2) <
							weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex1))			&&
							(weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex2) <
							weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex3))			) {
					closestToRay2.push_back(borderVertex);
				} else if	((weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex3) <
							weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex1))			&&
							(weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex3) <
							weExComputeShortestDistanceBetweenPointAndRay(borderVertex, finalTetraederTop, finalRayVertex2))			) {
			...
			for(auto vertexClosestToRay1 : closestToRay1){
				...
				weExComputeSquaredDistanceFromTetraederTopToProjectedPointOnLine(vertexClosestToRay1, finalTetraederTop, finalRayVertex1, currentDistanceRay1);
			...
			for(auto vertexClosestToRay2 : closestToRay2){
				...
				weExComputeSquaredDistanceFromTetraederTopToProjectedPointOnLine(vertexClosestToRay2, finalTetraederTop, finalRayVertex2, currentDistanceRay2);
			...
			for(auto vertexClosestToRay3 : closestToRay3){
				...
				weExComputeSquaredDistanceFromTetraederTopToProjectedPointOnLine(vertexClosestToRay3, finalTetraederTop, finalRayVertex3, currentDistanceRay3);
			...
			*/
			...
			TetraederHeights.push_back(weExComputeTetraederHeight(foundTetraeder));
	...
	weExWriteExtractedTetraedersIntoFile(extractedTetraeders, fileBaseName);
	...
	weExWriteExtractedTetraedersIntoMeshForCollection(meshIntrinsicExtractedTetraeders, RANSACQualities, TetraederHeights, extractedTetraeders);
	...
	weExNormalizeValues(RANSACQualities, normalizedRANSACQualities);

	vector<double> normalizedTetraederHeights;
	weExNormalizeValues(TetraederHeights, normalizedTetraederHeights);

	//Parameters: the 1 is the mode for RANSAC Quality visualization and 2 ist the mode for tetraeder height visualization.
	weExWriteExtractedTetraedersAndMeshIntoFile(extractedTetraeders, fileBaseName, normalizedRANSACQualities, 1, mVertices, mFaces);
	weExWriteExtractedTetraedersAndMeshIntoFile(extractedTetraeders, fileBaseName, normalizedTetraederHeights, 2, mVertices, mFaces);
	...
	cout << "Resetting feature vector to original length of 16 entries" << endl;

		//loop over all vertices
	for(auto pVertex : mVertices){

		//if the feature vector has more than 16 elements, resize it to length 16

		if(pVertex->getFeatureVectorLen() > 16){
			pVertex->resizeFeatureVector(16);
		}
	}
...
//! Additional input given by user to further change Non Maximum Suppression with a threshold or basin merging in the Watershed algorithm
bool Mesh::funcWeExAdditionalInput(double additionalInput){

	NMSWaterAdditionalInput = additionalInput;

	cout << "Non Maximum Suppression Threshold and Watershed Basin Merging Value set to " << additionalInput << endl;
